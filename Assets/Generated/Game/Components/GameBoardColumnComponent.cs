...
public partial class GameContext {
    ...
    public GameEntity boardColumnEntity { get { return GetGroup(GameMatcher.BoardColumn).GetSingleEntity(); } }
    public BoardColumnComponent boardColumn { get { return boardColumnEntity.boardColumn; } }
    public bool hasBoardColumn { get { return boardColumnEntity != null; } }

    public GameEntity SetBoardColumn(int newValue) {
        if (hasBoardColumn) {
            throw new Entitas.EntitasException("Could not set BoardColumn!\n" + this + " already has an entity with BoardColumnComponent!",
                "You should check if the context already has a boardColumnEntity before setting it or use context.ReplaceBoardColumn().");
        }
        var entity = CreateEntity();
        entity.AddBoardColumn(newValue);
        return entity;
    }

    public void ReplaceBoardColumn(int newValue) {
        var entity = boardColumnEntity;
        if (entity == null) {
            entity = SetBoardColumn(newValue);
        } else {
            entity.ReplaceBoardColumn(newValue);
        }
    }

    public void RemoveBoardColumn() {
        boardColumnEntity.Destroy();
    }
}
